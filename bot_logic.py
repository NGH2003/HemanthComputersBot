...
CHANNEL_ID = "@YOUR_CHANNEL_ID" # <--- ADD YOUR CHANNEL ID
...
async def suggest_opportunities(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not matched: await query.message.reply_text(f"🔍 No matches found."); return
...
async def run_bot():
    ...
    while True: await asyncio.sleep(3600# --- 3. LISTING & HANDLERS ---
async def show_listings(update: Update, category):
    query = update.callback_query
