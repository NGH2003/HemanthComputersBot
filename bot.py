...
# 🔴 FIX: Allow SQLite to work across multiple threads
...
<body class="bg-light p-4"><h2>🤖 HC Bot Admin</h2>
<div class="row mb-4"><div class="col-md-6"><div class="card p-3"><h3>{{ jobs }}</h3><small>Jobs</small></div></div>
<div class="col-md-6"><div class="card p-3"><h3>{{ users }}</h3><small>Users</small></div></div></div>
<div class="card"><div class="card-header">Sources</div><div class="card-body">
<ul class="list-group mb-3">{% for s in sources %}<li class="list-group-item">{{ s.url }} <a href="/del/{{ s.id }}" class="btn btn-danger btn-sm">X</a></li>{% endfor %}</ul>
<form action="/add" method="post"><div class="input-group"><input name="url" class="form-control" placeholder="RSS URL"><button class="btn btn-success">Add</button></div></form>
</div></div></body></html>
"""
...
@login_required
def add():
    if request.form.get('url'): db.session.add(Source(url=request.form.get('url'))); db.session.commit()
    return redirect('/admin')
...
def ask_district(message):
    ...
    if message.text != "Any Qualification":
...
def show_results(message):
    try:
        ...
        # Store District
        user_sessions[user_id]["dist"] = message.text
        if message.text != "Any District":
            user_sessions[user_id]["filters"].append(message.text)
    ...
    try:
        ...
        with app.app_context():
            ...
            if job:
                ...
                data = json.loads(job.ai_analysis) if job.ai_analysis else {}
